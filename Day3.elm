...
import Html exposing (p, text, div, h1, h2)
import Dict
import String exposing (toList, contains)
...
moveSleigh : List Char -> (Int, Int) -> Dict.Dict (Int, Int) Bool -> Dict.Dict (Int, Int) Bool
moveSleigh instructions currentPos seen =
  case instructions of
    first :: rest ->
      moveSleigh rest (nextPos currentPos first) (Dict.insert currentPos True seen)
    [] ->
      seen

housesVisitedPart1 : List Char -> Int
housesVisitedPart1 instructions = 
  let
    start = (0, 0)
  in
    Dict.size (moveSleigh instructions start Dict.empty)

-- Split instructions between santa and robot santa
moveSleigh2 idx instructions pos1 pos2 seen =
  case instructions of
    first :: rest ->
      if idx % 2 == 1 then
        moveSleigh2 (idx + 1) rest (nextPos pos1 first) pos2 (Dict.insert pos1 True seen)
      else
        moveSleigh2 (idx + 1) rest pos1 (nextPos pos2 first) (Dict.insert pos2 True seen)
    [] ->
      seen
     

housesVisitedPart2 : List Char -> Int
housesVisitedPart2 instructions =
  let
    seen = moveSleigh2 0 instructions (0, 0) (0, 0) Dict.empty
  in
    Dict.size seen
...
main =
  let
    instructions = toList input
  in
    div []
      [ h1 [] [ text "Day 3" ]
      , h2 [] [ text "Part 1" ]
      , p [] [ text (toString (housesVisitedPart1 instructions)), text " houses visited." ]
      , h2 [] [ text "Part 2" ]
      , p [] [ text (toString (housesVisitedPart2 instructions)), text " houses visited." ]
      ]
